...
token = "1087522005:AAHao6DOLoZ_t5LUMPKoDOrZMTBK8seHKiE"
bot = telebot.TeleBot(token)


@bot.message_handler(commands=["help"])
def helping(message):
    bot.send_message(message.chat.id, "Выбери пункт /translate, чтобы перевести текст или пункт /languages, чтобы посмотреть доступные языки")
...
def inlin(c):
    if c.data == "RU_EN":
        ...
        bot.send_message(c.message.chat.id, "Введите требуемый язык перевода на английском, либо воспользуйтесь /languages, чтобы посмотреть доступные языки")
